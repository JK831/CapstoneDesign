...
namespace Server.DB
{
    public class DbManager
    {
        ...
        {
            string connectString = string.Format("Server={0};Database={1};Uid ={2};Pwd={3};", DB_IP, DB_TARGET, DB_UID, DB_PWD);
            string sql = String.Format("Select Uid, SUM(Star), Rank_Table.ranking from (Select Uid, RANK() OVER (ORDER BY SUM(Star) DESC) AS ranking from '{0}' group by Uid) Rank_Table limit 30", Challenge_DB_Table);
            ...
            using (conn = new MySqlConnection(connectString))
            {
                ...
                while (reader.Read())
                {
                    list.Add(new S_ChallengeTop30.Rank()
                    {
                        ...
                        totalStars = Convert.ToByte(reader["SUM(Star)"])
                    });
                }
            }
            return list;
